...
export const sendAlarmEmail = async (req, res) => {
  ...
  try {
    ...
    // Buscar todas las transacciones del tipo "Retiro"
    const transactions = await TransactionHistoryModel.findAll({
      where: {
        transaction_type: "Retiro",
        amount: {
          [Op.gte]: 2001, // Filtrar retiros mayores o iguales a 200
        },
      },
    });
    ...
    if (!transactions.length) {
      return res.status(200).json({
        message: "No hay alarmas.",
      });
    }

    // Iterar sobre las transacciones y enviar un correo por cada una
    for (const transaction of transactions) {
      ...
      if (!account) {
        console.warn(
          `Cuenta asociada con ID ${transaction.account_id} no encontrada para la transacción ${transaction.id}.`
        );
        continue; // Omitir esta transacción si la cuenta no existe
      }
    ...
    return res.status(200).json({
      message: `Se enviaron ${transactions.length} correos de alarma por transacciones de retiro.`,
    });
  } catch (error) {
    console.error("Error al enviar alarmas por correo:", error);
    return res.status(500).json({
      message: "Ocurrió un error al enviar las alarmas por correo.",
    });
  }
};